...
public:
    ...
    Node(int id_, bool is_slow_ , ld Ttx_,ld Tk_, ld hash_power_ , Block* genesis_blk_);
    ~Node() = default;
    ...
    bool recv_blk(int peer_id, Block* B, simulator* simul,bool stop_mining);
    void mining_success(Block* B,simulator* simul,bool stop_mining);
    void printTree(int block_id, const std::unordered_map<int, std::vector<int>>& children, int depth = 0);
    void printBlockTree();
